        ...
        <Box sx={{ display: 'flex', flexDirection: "column", alignItems: "center", flexGrow: 1 }}>
            ...
            <Box component="form" sx={{ width: '100%' }} onSubmit={aoSubmeterForm}>
                ...
                <FormControl margin="dense" fullWidth>
                    ...
                    <Select id="select-tag" value={tag} onChange={evento => setTag(evento.target.value)}>
                        {tags.map(tag => <MenuItem key={tag.id} value={tag.value}>{tag.value}</MenuItem>)}
                    </Select>
                </FormControl>
                <FormControl margin="dense" fullWidth>
                    <InputLabel id="select-restaurante">Restaurante</InputLabel>
                    <Select id="select-restaurante" value={restaurante} onChange={evento => setRestaurante(evento.target.value)}>
